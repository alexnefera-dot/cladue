...
wb=openpyxl.load_workbook(io.BytesIO(open('launches20.xlsx','rb').read()),data_only=True)
SH=[('NEW33_12pages_nodate+img_25.08 ','ТЕСТ F · без дат','F','без дат','NEW33 · 12 стр · чужой контент + картинки'),
    ('NEW33_12pages_withdate+img_25.0','ТЕСТ F · с датами','F','с датами','NEW33 · 12 стр · чужой контент + картинки'),
    ('NEW17_NOimg_withdate_26.08','ТЕСТ E · без картинок','E','без картинок','NEW17 · чужой контент с датами'),
    ('NEW17_img_withdate_26.08','ТЕСТ E · с картинками','E','с картинками','NEW17 · чужой контент с датами'),
    ('КОНТРОЛЬ NEW50_5_7pages_nodate_','КОНТРОЛЬ','CTRL','контроль','7 стр без дат · прогон 21.08, третья дата'),
...
out=[]
for sn,name,test,arm,cfg in SH:
    ...
    for h in snh:
        ...
        doms=[]
        for d in hdr:
            ks=per[d]; top=sorted([x for x in ks if x['p']<=10],key=lambda x:(x['p'],-x['v']))[:8]
            doms.append({'d':d,'tm':d.endswith('.team'),'t3':sum(1 for x in ks if x['p']<=3),
              ...
              'nb':len({x['b'] for x in ks}),'keys':[{'q':x['q'],'p':x['p'],'b':x['b'],'t':x['t']} for x in top]})
        doms.sort(key=lambda x:(-x['t10'],-x['t30']))
        snaps.append(dict(lab=str(rows[h-1][0]).replace('Снимок ','').replace(' XML',''),quart=quart,corelen=len(b),
          ntm=len(ds),mean=round(sum(v)/n,3),med=st.median(v) if v else 0,
          wo=round((sum(v[1:])/(n-1)) if n>1 else 0,3),vals=v,
          ...
          nbrands=len({x['b'] for d in ds for x in per[d] if x['p']<=10}),doms=doms,
          per10={d:sum(1 for x in per[d] if x['p']<=10) for d in hdr}))
    out.append(dict(name=name,test=test,arm=arm,cfg=cfg,sheet=sn.strip(),ndom=len(hdr),snaps=snaps))
json.dump({'g':out,'file':'launches_20260827_122127.xlsx'},open('n3.json','w'),ensure_ascii=False)
print(f"{'ветка':24s} {'четверти с1':>18s} {'четверти с2':>18s}")
for r in out:
    print(f"{r['name'][:23]:24s} {str(r['snaps'][0]['quart']):>18s} {str(r['snaps'][1]['quart']):>18s}")
print()
print(f"{'ветка':24s} {'n':>2s} | {'с1 сред':>7s} {'с1 безлид':>9s} {'с1 Т30':>6s} | {'с2 сред':>7s} {'с2 безлид':>9s} {'с2 Т30':>6s} | значения с1 -> с2")
for r in out:
    a,b=r['snaps']
    print(f"{r['name'][:23]:24s} {a['ntm']:2d} | {a['mean']:7.2f} {a['wo']:9.2f} {a['t30']:6d} | {b['mean']:7.2f} {b['wo']:9.2f} {b['t30']:6d} | {a['vals']} -> {b['vals']}")
