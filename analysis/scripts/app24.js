...
function tabOverview(){
  const A=D.arms, i=A[0], n=A[1];
  const r=(x,y)=>y===0?'—':f1(x/y);
  return `<div class="blk"><h2>Четыре ветки, два съёма</h2>
  <p class="note">Т10/дом, медиана и «без лидера» — по .team-подмножеству
  (у теста B по две пары .lol на ветку, они идут отдельно ниже).
  «Доля лидера» — какую часть ключей группы держит её лучший домен.</p>
  <div class="tw"><table>${HEAD}<tbody>${D.groups.map(grow).join('')}</tbody></table></div></div>

  <div class="blk"><h2>Картинки: обе схемы вместе, 19 против 19</h2>
  <p class="note">Картинки сбалансированы внутри обоих тестов, поэтому ветки складываются.
  Тест B — 9 доменов .team на сторону, тест C — по 5 в каждом блоке, итого 19.</p>
  <div class="tw"><table>${HEAD}<tbody>${D.arms.map(grow).join('')}</tbody></table></div>
  <div class="cards" style="margin-top:14px">
    <div class="card ok"><h3>Без картинок впереди по всем мерам</h3>
      <p>Среднее <span class="big">${f2(n.t10)}</span> против ${f2(i.t10)},
      «без лидера» <b>${f2(n.wo)}</b> против ${f2(i.wo)}, медиана ${n.med} против ${i.med}.</p>
      <p>ТОП-3 <b>${n.t3}</b> против ${i.t3}, дорогих ключей
      <b>${n.vch+n.sch}</b> против ${i.vch+i.sch}.</p>
      <p class="mut">«Без лидера» вдвое выше — значит растёт вся ветка, а не один чемпион.</p></div>
    <div class="card err"><h3>Двенадцать часов назад было наоборот</h3>
      <p>На первом съёме картинки вели вдвое: 1,32 против 0,63 по тем же 38 доменам.
      За 12 часов ветка без картинок выросла в 13 раз, ветка с картинками — втрое.</p>
      <p><b>Один день ничего не решает.</b> Разброс между партиями одного формата
      в прошлых наблюдениях доходил до 13 раз. Вердикт — после 4-5 дней запусков.</p></div>
    <div class="card"><h3>Пары .lol идут в другую сторону</h3>
      <p>С картинками 2 и 2 ключа, без картинок 1 и 0. Направление обратное основному,
      но это два домена на сторону — шум.</p></div>
  </div></div>

  <div class="blk"><h2>Тест C: старые аккаунты против новых</h2>
  <p class="note">Блок 1 — старые вебмастер-аккаунты, блок 2 — новые. Запущены вместе,
  зона одна, картинки внутри каждого блока зеркальны 5 на 5.
  Это единственный чистый тест аккаунтов за всё наблюдение.</p>
  <div class="tw"><table>${HEAD}<tbody>${D.blocks.map(grow).join('')}</tbody></table></div>
  <div class="cards" style="margin-top:14px">
    <div class="card"><h3>По объёму новые, по типичному домену — ничья</h3>
      <p>Среднее <b>8,40 против 6,00</b> в пользу новых, но <b>«без лидера» 4,33 против 4,22</b>
      — то есть равно. Медиана 3,5 против 3,0.</p>
      <p>Весь отрыв даёт <span class="num">nchg.team</span> с 45 ключами. Убрать по два
      лидера с каждой стороны — 3,25 против 2,75, разрыв почти исчезает.</p></div>
    <div class="card ok"><h3>Новые взяли дорогие ключи</h3>
      <p>ВЧ+СЧ <b>9 против 1</b>, брендов <b>35 против 24</b>, Т100 517 против 434.
      Но семь из девяти дорогих держит тот же <span class="num">nchg.team</span>.</p>
      <p class="mut">ТОП-3 при этом чуть выше у старых: 18 против 16.</p></div>
    <div class="card"><h3>Старые вошли в индекс быстрее</h3>
      <p>На первом съёме, в два часа жизни, у старых было <b>81 ключ в сотне</b> против 37
      и <b>два пустых домена</b> против четырёх.</p>
      <p>За ночь новые догнали и обогнали. Аккаунт влияет на скорость входа
      в индекс, а не на потолок — по крайней мере на этих сутках.</p></div>
    <div class="card err"><h3>Возможное взаимодействие с картинками</h3>
      <p>У новых аккаунтов ветки разошлись вчетверо: без картинок <b>12,60</b>,
      с картинками 4,20. У старых почти не разошлись: 6,80 против 5,20.</p>
      <p>По пять доменов в ячейке — этого мало для вывода, но если картина
      повторится, эффект картинок зависит от возраста аккаунта.</p></div>
  </div></div>

  <div class="blk"><h2>Пришла вторая волна</h2>
  <p class="note">На съёме 22:24 дорогих ключей не было вообще ни у одного из 42 доменов.
  Через 12 часов их ${D.tot.hs}.</p>
  <div class="tw"><table><thead><tr><th class="l">Домен</th><th class="l">Ветка</th>
  <th class="l">Т10 по съёмам</th><th>ВЧ</th><th>СЧ</th><th class="l">Дорогие бренды</th>
  </tr></thead><tbody>
  ${DM.filter(d=>d.vch+d.sch>0).map(d=>`<tr><td class="l id">${esc(d.d)}</td>
    <td class="l mut">${esc(d.g)} · ${esc(d.arm)}</td>
    <td class="l num">${d.tr.join(' › ')}</td>
    <td class="${d.vch?'good':'mut'}">${d.vch}</td><td class="${d.sch?'good':'mut'}">${d.sch}</td>
    <td class="l num">${d.keys.filter(k=>k.t!=='НЧ').slice(0,6).map(k=>
      `${esc(k.b)} <span class="mut">${k.t} поз.${k.p}</span>`).join(', ')}</td></tr>`).join('')}
  </tbody></table></div></div>`;
}
