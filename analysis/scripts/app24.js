...
function tabOverview(){
  ...
  <div class="blk"><h2>Тест C: вебмастера, блок против блока</h2>
  <p class="note">Оба блока запущены вместе, зона одна, картинки внутри зеркальны.
  Чем блоки отличаются по типам аккаунтов — в реестр не передано,
  поэтому вопрос теста пока не считается.</p>
  <div class="tw"><table>${HEAD}<tbody>${D.blocks.map(grow).join('')}</tbody></table></div>
  <div class="cards" style="margin-top:14px">
    <div class="card"><h3>По объёму блок 2, по типичному домену — ничья</h3>
      <p>Среднее 8,40 против 6,00, но <b>«без лидера» 4,33 против 4,22</b> — практически
      равно. Весь отрыв даёт <span class="num">nchg.team</span> с 45 ключами.</p></div>
    <div class="card ok"><h3>Оба блока проснулись за ночь</h3>
      <p>На первом съёме было 0,00 и 0,10 ключа на домен и шесть пустых доменов на двоих.
      Сейчас <b>пустых нет ни одного</b>, и оба блока обогнали ветку с картинками теста B.</p>
      <p class="mut">Вчерашнее отставание теста C от теста B было чистым возрастом:
      15 часов против 18.</p></div>
    <div class="card"><h3>Дорогие ключи у блока 2</h3>
      <p>ВЧ+СЧ <b>9</b> против 1. Но семь из девяти держит тот же
      <span class="num">nchg.team</span>.</p></div>
  </div></div>
