...
def main(panel, subs_paths, out_csv):
    ...
    for r in rows:
        ...
        t['days'].add(r['day']); t['tld'][r['tld']] += 1
        d = day[r['day']]
    ...
    with open(out_csv, 'w', encoding='utf-8', newline='') as f:
        w = csv.writer(f)
        w.writerow(['набор', 'баз', 'сайтов', 'дней', 'зоны', 'вышли в поиск',
                    'поисковых кликов', 'регистраций', 'ФД',
                    'кликов на регистрацию', 'ожидалось регистраций', 'вердикт'])
        for name, t in order:
            w.writerow([name, len(t['bases']), t['n'], len(t['days']),
    ...
    print(f'-> {out_csv}\n')
    print(f"{'набор':<40}{'баз':>4}{'сайтов':>8}{'поиск':>7}{'кликов':>8}"
          f"{'рег':>4}{'ФД':>3}{'кл/рег':>8}{'ожид':>6}  вердикт")
    for name, t in order:
        print(f'{name[:39]:<40}{len(t["bases"]):>4}{t["n"]:>8}{t["ya"]:>7}{t["cl"]:>8}'
              f'{t["reg"]:>4}{t["fd"]:>3}'
