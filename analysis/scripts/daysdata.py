...
for names,g in D2708.items():
    for x in names.split(): DAY[x+'.team']='27.08'; GRP[x+'.team']=g
# запуск 03.09 — позиций ещё нет, но день существует
for x in open('/home/user/cladue/analysis/launch_03.09.txt'):
    x=x.strip()
    if x and not x.startswith('#'): DAY[x]='03.09'; GRP[x]='NEW50_3 styled (четыре ветки)'
def dat(s):
...
json.dump(dict(days=DAYS,gap=gap,arr=ARR,age=AGE,sim=SIM,curve=CURVE,
