...
function tabOverview(){
  ...
  return `<div class="blk" style="border-left:3px solid var(--warn);padding-left:14px">
  <h2>Исключены из анализа</h2>
  <p class="note"><span class="num">5374.team</span> (контроль, 30 ключей) и
  <span class="num">2535.team</span> (без картинок, 34 ключа) убраны из всех
  расчётов по вашей просьбе. Это меняет две цифры заметно: контроль 23,44 → 22,62,
  ветка «без картинок» дня 2 <b>8,50 → 4,86</b>, накопленное по картинкам
  8,41 → 7,42.</p></div>

  <div class="blk"><h2>Главное: выдача не просела</h2>
  ...
  <div class="cards">
    <div class="card ok"><h3>Контроль: ${f2(ctrl.t10)} ключа на домен за ~6 часов</h3>
      <p>Значения по восьми .team: <span class="num">${ctrl.vals.join(', ')}</span>.
      Лидер <span class="num">2424.team</span> — <b>108 ключей в ТОП-10</b>,
      27 в ТОП-3, 22 дорогих. <span class="mut">5374.team исключён.</span></p>
    ...
    <div class="card"><h3>Чем это подтверждается</h3>
      ...
      дал ${f2(D.groups[4].t10)} на шести часах. Слабее вилки, но втрое выше 24.08.</p>
  ...
  день 2 — 5 против 7. День 2 собран менее чисто: ветки вышли из разных прогонов
  с разрывом в 27 минут. <b>Домены 5374.team и 2535.team исключены из всего анализа
  по вашей просьбе</b> — 2535 был лидером ветки «без картинок» дня 2.</p>
  ${T(D.img)}
  <div class="cards" style="margin-top:14px">
  ...
  <div class="blk"><h2>Аккаунты: тест C на генераторе, тест D на наборах</h2>
  <p class="note">Два независимых захода на один вопрос. Тест C — 11 страниц нашего
  генератора, 10 против 10, возраст 15 часов. Тест D — наборы, 5 против 5,
  возраст около часа: цифры там сырые, но симметрия уже говорит.</p>
  ${T([D.groups[8],D.groups[7],D.groups[1],D.groups[2]])}
  <div class="cards" style="margin-top:14px">
    <div class="card"><h3>Тест C: на типичном домене ничья</h3>
      <p>Среднее 8,40 у новых против 6,00 у старых, но <b>«без лидера» 4,33 против 4,22</b>.
      Весь отрыв даёт <span class="num">nchg.team</span> с 45 ключами.</p>
      <p class="mut">ТОП-3 при этом выше у старых: 18 против 16.</p></div>
    <div class="card err"><h3>Тест D не повторил преимущество старых на старте</h3>
      <p>Гипотеза после теста C была: старые аккаунты быстрее входят в индекс —
      там на первом съёме у них было 81 ключ в сотне против 37.</p>
      <p>Здесь на первом съёме <b>ТОП-100 ровно 240 против 240</b>, ТОП-30
      <b>25 против 25</b>. Симметрия до единицы. Первое наблюдение против гипотезы.</p></div>
    <div class="card"><h3>Накопленно 15 против 15</h3>
      <p>Старые ${f2(D.acc[0].t10)} против новых ${f2(D.acc[1].t10)} на домен,
      «без лидера» ${f2(D.acc[0].wo)} против ${f2(D.acc[1].wo)},
      ТОП-3 ${D.acc[0].t3} против ${D.acc[1].t3}.</p>
      <p class="mut">Возрасты разные — 15 часов против часа — поэтому пул считается
      как ориентир, а не как вывод.</p></div>
    <div class="card"><h3>Наборы ещё не раскрылись</h3>
      <p>2,80 и 2,20 на домен при вилке формата <b>13,4…17,4</b> на 9-10 часах.
      Судить по ним рано, нужен двенадцатичасовой съём.</p></div>
  </div></div>
