...
function tSum(){
  ...
  <div class="card warn-c"><h3>Поправка к прежнему выводу про страницы</h3>
  <p>Раньше я приводил разрыв <b>27,7 % против 12,0 %</b> в пользу двенадцати страниц
  и называл его подтверждённым. Это оказалось ошибкой счёта: в тот расчёт попали домены,
  запущенные 19 и 20 августа, у которых первые сутки жизни не покрыты выгрузкой конверсий.
  Семистраничных групп среди них непропорционально много —
  ${t7.n} доменов против ${t12.n} двенадцатистраничных, —
  и обрезка данных давила именно семистраничную половину.</p>
  <p>На чистых закрытых окнах разрыв съёживается до
  <b>${nf(100*c12.w/c12.n,1)} % против ${nf(100*c7.w/c7.n,1)} %</b>:
  ${c12.w} из ${c12.n} против ${c7.w} из ${c7.n}. Точный тест Фишера даёт <b>p = 0,19</b>.
  Вместе со свежей проверкой на запусках 4–6 сентября (5 из 30 против 1 из 30) —
  <b>${nf(100*(c12.w+5)/(c12.n+30),1)} % против ${nf(100*(c7.w+1)/(c7.n+30),1)} %, p = 0,067</b>:
  направление держится и с новыми данными подтянулось, но порога всё ещё не перешло.</p>
  <p><b>Как это читать.</b> Двенадцать страниц по-прежнему лучший кандидат
  и на них стоит ставить, пока нет ничего лучше. Но называть это доказанным рычагом
  нельзя — нужен ещё один-два таких же чистых набора, как ветки NEW20/NEW21/NEW20_3.</p></div>

  ${barTable('Число страниц',pgC,
    'Одиннадцать страниц здесь даже выше двенадцати — а это генераторные группы. '+
    'Разброс между всеми тремя значениями укладывается в собственную погрешность каждого.')}
  ...
  <p class="verd">Ни один из четырёх признаков не даёт разрыва, который бы уверенно
  вышел за погрешность. Единственный рычаг, который держится на этих данных, —
  <b>класс бренда</b>: ВЧ-бренды дают на порядок больше регистраций на ключ в первой десятке,
  чем НЧ. Всё остальное, включая страницы, пока кандидаты, а не выводы.</p></div>`;
}
