...
/* ---------- tab: новые группы ---------- */
function tabNew(){
  return `<div class="blk"><h2>Шесть групп на подходе</h2>
  ...
  <th>Дом</th><th>.team</th><th>.buzz</th><th class="l">прочие</th></tr></thead><tbody>
    ...
    <tr><td class="l id">Группа 2</td><td>7</td><td class="l">генератор</td>
      <td class="l">v4</td><td class="l warn">не указан</td>
    ...
    <tr><td class="l id">Группа 5</td><td>7</td><td class="l">генератор</td>
      ...
      <td>5</td><td>5</td><td>0</td><td class="l mut">—</td></tr>
    <tr><td class="l id">Группа 6</td><td>7</td><td class="l">генератор #2</td>
      <td class="l">v4_2</td><td class="l"><b>Theme1</b></td>
      <td>5</td><td>4</td><td>1</td><td class="l mut">—</td></tr>
    <tr class="tot"><td class="l">Итого</td><td>—</td><td class="l">40 доменов</td>
      <td class="l">—</td><td class="l">—</td>
      <td>40</td><td>30</td><td>5</td><td class="l">5 экзотических</td></tr>
  </tbody></table></div></div>

  <div class="blk"><h2>Шаблон Группы 2 решает половину теста</h2>
  <p class="note">Группа 5 — это v5 + Theme2, Группа 6 — v4_2 + Theme1. Они отличаются
  сразу по двум параметрам, поэтому напрямую друг с другом не сравниваются.
  Обе развязки идут через Группу 2, у которой шаблон пока неизвестен.</p>
  <div class="cards">
    <div class="card ok"><h3>Если Группа 2 — это Theme2</h3>
      <p><b>Версия генератора измеряется:</b> Гр.2 (v4, Theme2, 6 доменов .team)
      против Гр.5 (v5, Theme2, 5 доменов). Одна тема, 7 страниц у обеих, разная версия.</p>
      <p><b>Шаблон тоже измеряется:</b> Гр.2 (Theme2) против Гр.6 (Theme1),
      обе на генераторе v4 — с оговоркой на разницу v4 и v4_2.</p></div>
    <div class="card err"><h3>Если Группа 2 — это Theme1</h3>
      <p>Ни версия, ни шаблон не измеряются. Пар с одним изменённым параметром
      в тесте не остаётся.</p>
      <p>Работать будут только два сравнения: источник контента (Гр.1 vs Гр.2)
      и объём страниц (Гр.2 vs Гр.3+4).</p></div>
    <div class="card acc"><h3>Повтор конфигурации</h3>
      <p>Гр.2 (v4) и Гр.6 (v4_2) — обе 7 страниц на генераторе v4. При совпадении шаблонов
      это первый в архиве прямой повтор конфигурации на новых доменах.</p>
      <p>По архиву воспроизводимость низкая: donor-2 повторяли трижды (D259, D262, D264) — ноль.
      Повтор покажет, воспроизводится ли хотя бы уровень конфигурации.</p></div>
  </div></div>

  <div class="blk"><h2>Что тест покажет в любом случае</h2>
  <div class="cards">
    <div class="card ok"><h3>Источник контента — чисто</h3>
      ...
      <p class="mut">Единственное сравнение теста без поправок. Полными группами, 10 против 10.</p></div>
    <div class="card acc"><h3>Объём страниц — считать по .team</h3>
      <p>9 доменов .team из групп 3+4 против 6 доменов .team из группы 2.
      Полными группами нельзя: 11-страничные почти целиком .team, группа 2 — только 6 из 10.</p>
  ...
  <ul class="cl">
    <li><b>Шаблон Группы 2 — Theme1 или Theme2?</b> Самый ценный неизвестный параметр:
      от него зависят сразу два сравнения из четырёх.</li>
    <li><b>Шаблон групп 1, 3, 4.</b> Для пары Гр.1 vs Гр.2 не критично — если у обеих
      он одинаковый, сравнение остаётся чистым независимо от того, какой именно.</li>
    <li><b>Версия генератора у групп 3 и 4.</b> Контенты Гр.4 и Гр.5 созданы в одну минуту
      (22:39 и 22:40), Гр.3 — в 16:56. Если 11-страничные тоже v5, объём страниц
      сравнивается внутри одной версии.</li>
    <li><b>Generator_11page и Generator_11page_2 — одни настройки?</b> Если да,
      группы 3 и 4 пулятся в 10 доменов; если нет — обе остаются по 5.</li>
  </ul></div>
  ...
  <ul class="cl">
    ...
    <li><b>Вклад версии и шаблона по отдельности</b>, если Группа 2 окажется Theme1.
      Четыре переменные на 40 доменов — это больше, чем выборка способна развести.</li>
    <li><b>Работу экзотических зон.</b> Пять зон по одному домену. .cyou в архиве
      не встречается ни разу. Смотреть Т30, а не факт входа в ТОП-10.</li>
  ...
  <ul class="cl">
    <li><b>Гр.1 vs Гр.2</b> — полными группами, 10 против 10. Источник контента.</li>
    <li><b>Гр.2(.team) vs Гр.3+4(.team)</b> — 6 против 9. Объём страниц.</li>
    <li><b>Гр.2(.team) vs Гр.5</b> — 6 против 5. Версия генератора, только если Гр.2 = Theme2.</li>
    <li><b>Гр.2(.team) vs Гр.6(.team)</b> — 6 против 4. Шаблон, только если Гр.2 = Theme2.</li>
...
function tabM1(){
  const M=D.m1, ord=["группа 3","группа 1","группа 4","группа 5","группа 6","группа 2"];
  const T=(d)=>d.endsWith(".team");
  const sub=(sn,p)=>{const g=M[sn].doms.filter(x=>p?p(x.d):true), n=g.length;
    return {n, t30:g.reduce((a,x)=>a+x.t30,0)/n, t100:g.reduce((a,x)=>a+x.t100,0)/n,
            vch:g.reduce((a,x)=>a+x.vch,0), t3:g.reduce((a,x)=>a+x.t3,0)};};
  const cmp=(title,note,rows)=>`<div class="blk"><h2>${title}</h2>
    <p class="note">${note}</p><div class="tw"><table><thead><tr><th class="l">Сторона</th>
    <th>Дом</th><th>Т30/дом</th><th>Т100/дом</th><th>ВЧ Т10</th><th>Т3</th></tr></thead><tbody>`+
    rows.map(([nm,s,hi])=>`<tr><td class="l ${hi?'id':''}">${nm}</td><td>${s.n}</td>
      <td><b>${f1(s.t30)}</b></td><td>${f1(s.t100)}</td>
      <td class="${s.vch?'good':'mut'}">${s.vch}</td><td>${s.t3}</td></tr>`).join('')+
    `</tbody></table></div></div>`;

  let groups=`<div class="blk"><h2>Замер 1 · 20.08.2026</h2>
  <p class="note">Первый съём по всем шести группам. Второй блок на каждом листе выгрузки —
  точная копия первого, посчитан один раз. Ядро полное, 1570 ключей.</p>
  <div class="tw"><table><thead><tr><th class="l">Группа</th><th class="l">Конфигурация</th>
  <th class="l">Верс</th><th class="l">Шаблон</th><th>Дом</th><th>ВЧ Т10</th><th>СЧ Т10</th>
  <th>НЧ Т10</th><th>Т30/дом</th><th>Т100/дом</th><th>Есть Т30</th></tr></thead><tbody>`;
  ord.forEach(sn=>{const g=M[sn], m=g.meta, s=sub(sn);
    const vch=g.doms.reduce((a,x)=>a+x.vch,0), sch=g.doms.reduce((a,x)=>a+x.sch,0);
    const nch=g.doms.reduce((a,x)=>a+x.nch,0);
    const ent=g.doms.filter(x=>x.t30>0).length/g.n;
    groups+=`<tr class="clk" data-g="${sn}"><td class="l id">${m[0]}</td><td class="l">${m[1]}</td>
      <td class="l">${m[2]}</td><td class="l">${m[3]}</td><td>${g.n}</td>
      <td class="${vch?'good':'mut'}"><b>${vch}</b></td><td class="${sch?'good':'mut'}">${sch}</td>
      <td class="mut">${nch}</td><td><b>${f1(s.t30)}</b></td><td>${f1(s.t100)}</td>
      <td>${pill(ent,.9,.5)}</td></tr>
      <tr class="det" hidden><td colspan="11"><div class="inner"></div></td></tr>`;});
  groups+=`</tbody></table></div>
  <p class="note" style="margin-top:10px">Клик по строке — домены группы.
  ВЧ и СЧ на первом замере почти везде нули, это норма: по архиву дорогие бренды приходят
  второй волной. Рабочий показатель на этой стадии — <b>Т30 на домен</b>.</p></div>`;

  const calib=`<div class="blk"><h2>Что Т30 на первом замере значил в архиве</h2>
  <p class="note">16 запусков архива с тремя и более замерами. Корреляция Т30/дом на замере 1
  с финалом — 0,70. Разделение почти без пересечения.</p>
  <div class="tw"><table><thead><tr><th class="l">Группа архива</th><th>Запусков</th>
  <th>Т30/дом на замере 1</th><th>Медиана</th></tr></thead><tbody>
    <tr><td class="l good">Успешные (финал ≥ 2 ВЧ+СЧ/дом)</td><td>4</td>
      <td>12,8 – 28,6</td><td><b>22,3</b></td></tr>
    <tr><td class="l bad">Провальные (финал &lt; 0,5)</td><td>6</td>
      <td>3,0 – 8,4</td><td><b>4,9</b></td></tr>
  </tbody></table></div>
  <p class="note" style="margin-top:10px">На уровне доменов (n=330): Т30 ≥ 40 на замере 1 →
  88 % доходят до результата, медиана финала 10. Т30 от 15 до 39 → 54 %. Т30 ≤ 4 → 15 %.</p>
  <div class="tw" style="margin-top:14px"><table><thead><tr><th class="l">Новая группа</th>
  <th>Т30/дом</th><th class="l">Куда попадает</th></tr></thead><tbody>
    <tr><td class="l id">Группа 3</td><td><b>48,8</b></td>
      <td class="l good">Выше любого запуска архива на замере 1 (максимум был 28,6)</td></tr>
    <tr><td class="l id">Группа 1</td><td><b>10,3</b></td>
      <td class="l warn">В зазоре между провальными и успешными</td></tr>
    <tr><td class="l id">Группа 4</td><td>4,0</td><td class="l bad">Полоса провальных</td></tr>
    <tr><td class="l id">Группа 5</td><td>2,8</td><td class="l bad">Ниже всех провальных архива</td></tr>
    <tr><td class="l id">Группа 6</td><td>2,0</td><td class="l bad">Ниже всех провальных архива</td></tr>
    <tr><td class="l id">Группа 2</td><td>1,5</td><td class="l bad">Ниже всех провальных архива</td></tr>
  </tbody></table></div></div>`;

  const contradiction=`<div class="blk"><h2>Главное противоречие: Гр.3 против Гр.4</h2>
  <p class="note">Обе группы — 11 страниц на генераторе, обе почти целиком .team.
  Разница между ними в 12 раз. Значит объём страниц сам по себе результат Группы 3 не объясняет.</p>
  <div class="cards">
    <div class="card err"><h3>Что это НЕ доказывает</h3>
      <p>«11 страниц работают». Группа 4 — тоже 11 страниц, и она на уровне
      семистраничных: <span class="big">4,0</span> против 48,8 у Группы 3.</p></div>
    <div class="card"><h3>Версия 1: возраст</h3>
      <p>Контенты Гр.3 созданы 19.08 в 16:56, Гр.4/5/6 — в 22:39–22:50. Если сайты поднимались
      вместе с контентом, Гр.3 старше остальных примерно втрое (8,5 ч против 3 ч).</p>
      <p class="mut">Против этой версии: по записи о запуске Гр.3 наоборот младше —
      создана в 01:21 и снята в 01:29, то есть 8 минут против 10–11 у остальных.</p></div>
    <div class="card"><h3>Версия 2: партия контента</h3>
      <p>Generator_11page и Generator_11page_2 — разные наборы. По архиву разброс между партиями
      огромен и не предсказывается: чемпион попадается раз на 15–30 доменов.</p>
      <p class="mut">Пять доменов в каждой группе — этого мало, чтобы различить версии.</p></div>
  </div>
  <p class="note" style="margin-top:12px">Замер 2 разведёт версии: если Гр.4, 5 и 6 подтянутся
  по мере старения — дело было в возрасте; если Гр.3 сохранит отрыв — дело в контенте.</p></div>`;

  return groups+calib+
    cmp("Источник контента · 7 страниц",
      "Самое чистое сравнение теста: зоны совпадают точно (по 6 .team, по 2 .buzz, по 2 экзотических), запуски созданы в одну минуту и сняты в одну. Поправок не требует.",
      [["Гр.1 · контент из выдачи", sub("группа 1"), 1],["Гр.2 · генератор v4", sub("группа 2")]])+
    cmp("То же, только .team",
      "Экзотические зоны убраны — на них по одному домену, и они тянут обе стороны вниз.",
      [["Гр.1 из выдачи (.team)", sub("группа 1",T), 1],["Гр.2 генератор v4 (.team)", sub("группа 2",T)]])+
    contradiction+
    cmp("Объём страниц · только .team",
      "Две 11-страничные группы разведены — усреднять их до выяснения причины расхождения нельзя.",
      [["7 стр · Гр.2 (.team)", sub("группа 2",T)],["11 стр · Гр.3", sub("группа 3",T), 1],
       ["11 стр · Гр.4 (.team)", sub("группа 4",T)]])+
    cmp("Версия генератора и шаблон · только .team",
      "v4 против v5 против v4_2, Theme1 против Theme2. Разброс между тремя партиями — 2,5 / 2,8 / 2,5. Различий нет ни по версии, ни по шаблону.",
      [["Гр.2 · v4", sub("группа 2",T)],["Гр.5 · v5 · Theme2", sub("группа 5",T)],
       ["Гр.6 · v4_2 · Theme1", sub("группа 6",T)]])+
    cmp("Зоны внутри групп 1 и 2",
      "Контроль по группе — как в архивном срезе. .team выигрывает у экзотических зон в обеих группах.",
      [["Гр.1 · .team", sub("группа 1",T)],["Гр.1 · .buzz .quest .bond", sub("группа 1",d=>!T(d))],
       ["Гр.2 · .team", sub("группа 2",T)],["Гр.2 · .buzz .icu .top", sub("группа 2",d=>!T(d))]])+
    `<div class="blk"><h2>Выводы замера 1</h2>
    <ul class="cl">
      <li><b>Контент из выдачи бьёт генератор в 6,5 раза.</b> 16,2 против 2,5 Т30 на домен
        по .team, 4 ВЧ-ключа против нуля. Единственное сравнение теста без поправок,
        зоны и время съёма совпадают. Это самый надёжный результат на сегодня.</li>
      <li><b>Версия генератора и шаблон не дали ничего.</b> v4 — 2,5, v5 — 2,8, v4_2 — 2,5.
        Theme1 и Theme2 неразличимы. Вопрос про шаблон Группы 2 теряет остроту:
        разводить нечего.</li>
      <li><b>Про 11 страниц сказать нельзя ничего.</b> Две партии одного формата разошлись
        в 12 раз. Пока причина не выяснена, объём страниц не измерен.</li>
      <li><b>Зоны подтвердились в третий раз.</b> .team против экзотических: 16,2 против 1,5
        в Гр.1 и 2,5 против 0,0 в Гр.2. .icu, .top, .quest, .bond и .cyou — по нулю или почти.</li>
      <li><b>Ранжируется пока только НЧ.</b> 7 ВЧ-ключей на 40 доменов. По архиву это норма
        первого замера: дорогие бренды приходят второй волной.</li>
    </ul></div>
    <div class="blk"><h2>Что делать дальше</h2>
    <ul class="cl">
      <li><b>Снять замер 2.</b> Он разведёт возраст и контент в паре Гр.3 / Гр.4 — сейчас
        это главная неизвестная.</li>
      <li><b>Уточнить, когда сайты реально поднялись</b> по каждой группе. Если время подъёма
        совпадает с созданием контента, разрыв Гр.3 и Гр.4 объясняется возрастом,
        и сравнивать группы нужно будет с поправкой.</li>
      <li><b>Не списывать группы 2, 5, 6 сейчас.</b> Они ниже всех провальных запусков архива,
        но правило отсева работает с замера 2, а не с первого.</li>
      <li><b>Судить о тесте не раньше замера 3-4.</b> D273 на замере 2 стоял на 0,20 и был
        предпоследним в архиве — через замер стал первым.</li>
    </ul></div>`;
}
function fillM1(){
  document.querySelectorAll('tr.clk[data-g]').forEach(tr=>{
    tr.onclick=()=>{
      ...
      if(!det.hidden && !slot.dataset.done){
        slot.dataset.done=1;
        const g=D.m1[tr.dataset.g];
        slot.innerHTML='<h4>Домены — ВЧ / СЧ / НЧ в ТОП-10 · Т30 · Т100 · брендов в Т30</h4>'+
          '<div class="chips">'+g.doms.map(x=>{
            const c=x.vch>0?'v':x.sch>0?'s':'';
            return '<span class="chip '+c+'">'+esc(x.d)+' <b>'+x.vch+'/'+x.sch+'/'+x.nch+
              '</b> ·'+x.t30+' ·'+x.t100+' <span style="opacity:.6">'+x.nb+' бр.</span></span>';
          }).join('')+'</div>';
      }
    };
  });
...
/* ---------- shell ---------- */
const TABS=[["Итог",tabItog,null],["Замер 1",tabM1,fillM1],["Запуски",tabRun,fillRun],["Возраст",tabAge,null],
