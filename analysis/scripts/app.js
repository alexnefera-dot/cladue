...
/* ---------- tab: новые группы ---------- */
function tabNew(){
  return `<div class="blk"><h2>Три группы на подходе</h2>
  <p class="note">Позиций ещё нет. Ниже — что тест сможет показать, а что нет, посчитанное
  на архиве до того, как придут данные.</p>
  <div class="tw"><table><thead><tr><th class="l">Группа</th><th class="l">Контент</th>
  <th>Дом</th><th>.team</th><th>.buzz</th><th>прочие</th></tr></thead><tbody>
    <tr><td class="l id">Группа 1</td><td class="l">7 страниц, сайты из выдачи (10 контентов)</td>
      <td>10</td><td>6</td><td>2</td><td>2 <span class="mut">.quest .bond</span></td></tr>
    <tr><td class="l id">Группа 2</td><td class="l">7 страниц, generator v4 (один на всех)</td>
      <td>10</td><td>6</td><td>2</td><td>2 <span class="mut">.icu .top</span></td></tr>
    <tr><td class="l id">Группа 3</td><td class="l">11 страниц, generator (5 контентов)</td>
      <td>5</td><td>5</td><td>0</td><td>0</td></tr>
  </tbody></table></div></div>

  <div class="blk"><h2>Что тест покажет</h2>
  <div class="cards">
    <div class="card ok"><h3>Источник контента — чисто</h3>
      <p>Группа 1 против Группы 2: по 6 .team, по 2 .buzz, по 2 экзотических зоны в каждой.
      Состав совпадает точно, разница будет означать именно источник контента.</p>
      <p class="mut">Считать как есть, по полным группам.</p></div>
    <div class="card err"><h3>Объём страниц — загрязнено зоной</h3>
      <p>Группа 3 целиком .team, у Группы 2 только 6 из 10. Сравнение полных групп даст
      11 страницам фору за счёт зоны, а не объёма.</p>
      <p>Считать <b>6 доменов .team из Группы 2 против 5 доменов Группы 3</b>.</p></div>
    <div class="card"><h3>Зоны .icu / .top — только сигнал</h3>
      <p>По одному домену на зону. При базовом «зашло» 32 % один домен ничего не доказывает.
      Смотреть на Т30, а не на факт входа.</p></div>
  </div></div>

  <div class="blk"><h2>Чего тест не покажет</h2>
  <ul class="cl">
    <li><b>Разницу уровня «8,17 против 3,61» на пяти доменах поймать нельзя.</b> Чемпион в архиве
      встречается примерно раз на 15–30 доменов; на n=5 шанс поймать его около 20 %.
      Один такой домен сдвигает среднее по группе на +5–7.</li>
    <li><b>Результат Группы 3 будет решать один домен.</b> В D273 из 8,17 на домен
      55 ключей из 98 дал один rtnm.team. При пяти доменах любой вывод — это вывод про один сайт.</li>
    <li><b>Судить раньше третьего замера бессмысленно.</b> D273 на замере 2 стоял на 0,20 —
      предпоследним в архиве. Через замер стал первым.</li>
    <li><b>4 домена .buzz в группах 1–2 не нули</b>, но слабее .team в 2,6 раза. Они занижают
      обе группы одинаково, поэтому сравнению не мешают — но абсолютные цифры будут ниже
      «чистых» .team-партий.</li>
  </ul></div>

  <div class="blk"><h2>Что сделать до замеров</h2>
  <ul class="cl">
    <li><b>Долить Группу 3 до 10–12 доменов</b>, желательно с 2 .buzz — тогда сравнение объёма
      станет симметричным Группе 2 и не потребует выкидывания половины выборки.</li>
    <li><b>Снять минимум 4 замера</b> по каждой группе. Три — минимум, на котором виден рывок.</li>
    <li><b>На замере 2 смотреть Т30 на домен, а не ВЧ.</b> Порог интереса — от 15 Т30 на домен
      в среднем по группе; ниже 7 партия почти наверняка пустая.</li>
  </ul></div>`;
}
