...
На выходе строка на сабдомен:
    [сабдомен, всего, ботов, из поиска, из поиска не бот,
     первый поисковый клик, первый клик любой, последний клик любой,
     последний поисковый клик]

Даты нужны, чтобы считать срок жизни домена и задержку от клика до регистрации:
конверсия приходит позже клика, и без обеих дат разрыв не измерить.
...
def main(out, specs):
    ...
    for spec in specs:
        ...
        for line in open(path, encoding='utf-8'):
            ...
            a = agg.get(s)
            if a is None:
                agg[s] = a = [0, 0, 0, 0, None, None, None, None]
            a[0] += 1
            a[1] += bot
            if at:
                if a[5] is None or at < a[5]:
                    a[5] = at
                if a[6] is None or at > a[6]:
                    a[6] = at
            if ya:
                a[2] += 1
                if not bot:
