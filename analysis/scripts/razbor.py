...
Свежие запуски **не выбрасываются**. По правилу, о котором договорились, они
идут в расчёт наравне с остальными, но каждая строка помечена долей сайтов с
незакрытым окном: у них было меньше времени на конверсию, и на следующем
расчёте их числа вырастут.
...
def main(panel, subs_paths, events):
    ...
    closed = rows   # свежие не выбрасываются, а помечаются долей незакрытых
    ...
    def fresh(sel):
        return sum(1 for r in sel if not r['closed'])
    ...
    print('по зонам (все запуски, включая свежие):')
    print(f"{'зона':<10}{'сайтов':>9}{'свежих':>8}{'с кликами':>11}{'из поиска':>11}"
          f"{'с рег.':>8}{'с ФД':>7}{'рег':>6}{'ФД':>5}{'рег на 1000':>13}")
    ...
    for z, sel in sorted(byz.items(), key=lambda kv: -len(kv[1])):
        ...
        print(f'{str(z):<10}{n:>9}{fresh(sel):>8}{c:>11}{y:>11}{g:>8}{f:>7}{R:>6}{F:>5}'
              f'{1000.0*R/n:>13.2f}')

    print('\nпо месяцам переобхода:')
    print(f"{'месяц':<10}{'сайтов':>9}{'свежих':>8}{'с кликами':>11}{'из поиска':>11}"
          f"{'с рег.':>8}{'с ФД':>7}{'рег':>6}{'ФД':>5}{'рег на 1000':>13}")
    ...
    for m, sel in sorted(bym.items()):
        ...
        print(f'{m:<10}{n:>9}{fresh(sel):>8}{c:>11}{y:>11}{g:>8}{f:>7}{R:>6}{F:>5}'
              f'{1000.0*R/n:>13.2f}')
