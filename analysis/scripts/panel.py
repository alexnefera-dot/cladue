    ...
    for day, files in m.FILES.items():
        for f in files:
            for main, _extra, doms in m.parse(f):
                ...
                for d in doms:
                    # один домен встречается в нескольких файлах журнала: недельном
                    # и общем. Пустое имя из одного файла не должно затирать
                    # настоящее имя из другого — раньше затирало, и 94 базы
                    # оставались без пака при живом заголовке в журнале.
                    if name is None and d in out and out[d][0] is not None:
                        continue
                    if d in out and out[d][0] is not None and name is not None:
                        continue
                    out[d] = (name, day)
    return out
...
def _norm_content(h):
    """Имя пака из заголовка секции журнала.

    Срезается только номер экземпляра («_1…_50», «_44») и хвост после тире или
    запятой («— id 1557-1606»). Дата в имени пака сохраняется: `..._24.08` и
    `..._25.08` — это разные запуски, и прежняя версия склеивала их в один.

    Разделитель «·» приводится к пробелу, чтобы «КОНТРОЛЬ · NEW50_5…» и
    «КОНТРОЛЬ NEW50_5…» из разных файлов журнала были одним именем.

    Отбрасываются баннеры секций вида «ЗАПУСКИ 24.08.2026 — 42 домена»: это
    дата, а не пак.
    """
    if not h:
        return None
    ...
    s = re.sub(r'[_\s]*\d+\s*(?:…|\.{2,3})\s*_?\d*$', '', s)   # _1…_50, _1..._50
    s = re.sub(r'_\d+$', '', s)                                  # _44, _372373; дата
                                                                 # «_24.08» уцелеет: после
                                                                 # подчёркивания там не
                                                                 # только цифры
    s = s.strip(' ,;')
