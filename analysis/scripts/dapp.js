...
function tQ(){
  ...
  в <b>${nf(D.sim.p,1)}% случаев</b>. Раньше здесь было 17 % и вывод читался просто:
  случайность. С полной выгрузкой цифра ушла за порог — но целиком за счёт одного дня.
  <b>31 августа</b> стоит на ${nf(D.days.find(d=>d.day==='31.08')?.share||0,1)} %,
  вдвое выше остальных. Если его убрать, размах падает до 15,9 пункта, а вероятность
  такого размаха случайно — <b>53 %</b>.</p>
  <div class="card warn-c" style="margin-top:12px"><h3>Почему 31 августа — не «удачная дата»</h3>
  <p>Этот день целиком состоит из трёх экспериментальных веток, и ни одна из них
  не похожа на обычный запуск: <code>Generator_11page_test</code> на движке dorgen.com
  с потолком вложенности <code>/ru</code> = 20, <code>nabor-244…253</code> на dorgen.net
  <i>без потолка вообще</i>, и двухдоменный кастомный контент на 12 страниц.</p>
  <p>То есть выделяется не дата, а конфигурация: другой движок и снятое ограничение
  вложенности. Проверять это надо повторным запуском той же конфигурации в другой день,
  а не смотреть на календарь. Пока такого повтора не было — в остальные дни
  ни dorgen.com, ни «без потолка» не запускались.</p></div>
  ...
  <div class="grid2" style="margin-top:16px">
    ...
    <p>Полная выгрузка добавила 28 событий за 3–7 сентября и подняла 31 августа
    с 38 до 48 %. Вывод про даты от этого не изменился, но изменилась формулировка:
    дни различаются больше, чем позволяет случайность, и весь этот перевес сидит
    в одной конфигурации 31 августа, а не в дате.</p></div>
  </div></div>`;
}
