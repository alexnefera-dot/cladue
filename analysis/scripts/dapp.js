...
function tQ(){
  ...
  <div class="grid2" style="margin-top:16px">
    ...
    <p>Вывод от этого не поменялся: размах между днями как был, так и остался в пределах
    случайного. Удачных дат нет, и добавление двух новых дней это только подтвердило.</p></div>
  </div></div>`;
}
...
function tFresh(){
  ...
  ${pair(pg)}
  <p class="verd">Направление то же, что и в истории, и разрыв даже больше:
  <b>16,7 % против 3,3 %</b>. Но выборка маленькая, точный тест Фишера даёт
  <b>p = ${nf(F.fisher,2).replace('0,','0,')}</b> — по одному этому набору вывод бы не устоял.
  Ценность в том, что он совпал по знаку с историческим 27,7 % против 12,0 %,
  а это уже независимое подтверждение, а не то же самое измерение второй раз.
