...
def main():
    ...
    say(f"  {'m':>3s} {'страт':>5s} {'домен':>5s} {'|Δ| медиана':>11s} {'|Δ| 90%':>8s} {'|Δ| 95%':>8s} {'доля |Δ|≥5п':>12s} {'доля |Δ|≥10п':>13s}")
    probe = {}
    for m in (5, 8, 13):
        ss = [s for s in strata if s.k >= 2 * m and s.p >= P_LOW]
        if not ss:
            say(f"  {m:3d}     — (нет страт с k ≥ {2*m})")
            continue
        diffs = []
        for s in ss:
            idxs = list(range(s.k))
        ...
        probe[m] = (len(ss), sum(s.k for s in ss), median(diffs), quantile(diffs, .9), quantile(diffs, .95),
                    sum(1 for d in diffs if d >= 0.05) / len(diffs), sum(1 for d in diffs if d >= 0.10) / len(diffs))
        pr = probe[m]
        say(f"  {m:3d} {pr[0]:5d} {pr[1]:5d} {pr[2]*100:10.1f}п {pr[3]*100:7.1f}п {pr[4]*100:7.1f}п {pr[5]*100:11.1f}% {pr[6]*100:12.1f}%")
    ...
    if homog:
        ...
        say(f"   Различие семейств не доказано и не исключено (p от {fmt_p(p_fam_min)} до {fmt_p(p_fam_max)} в зависимости от нуля): "
            ...
            f"({len(fam['nabory'])} страт по 5–7 доменов, выход 1–16 %), и его низкое φ по большей части объясняется низким выходом (см. п. 2).")
    ...
    if pr5:
        say(f"   Прямая проверка на {pr5[0]} стратах с k ≥ 10: проба из 5 доменов расходится с остальными доменами ТОГО ЖЕ набора на ≥5 п в {pr5[5]*100:.0f} % выборок, "
            f"на ≥10 п — в {pr5[6]*100:.0f} %; медиана расхождения {pr5[2]*100:.1f} п, 95-й процентиль {pr5[4]*100:.1f} п. "
            + (f"Проба из 8: ≥5 п в {probe[8][5]*100:.0f} %, ≥10 п в {probe[8][6]*100:.0f} % ({probe[8][0]} страт). " if 8 in probe else "")
            + (f"Проба из 13: ≥5 п в {probe[13][5]*100:.0f} %, ≥10 п в {probe[13][6]*100:.0f} % ({probe[13][0]} страт)." if 13 in probe else ""))
    ...
    say(f"Итог: «φ одинакова у всех семейств» — {v1}; «φ не зависит от среднего выхода» — {v2} (ρ = {rho_phi:+.2f}, p = {fmt_p(p_rho_phi)}: "
        ...
        + (f", прямая проверка: {pr5[5]*100:.0f} % проб из 5 ошибаются на ≥5 п" if pr5 else "") + "); у наборов с выходом 15–20 % нужно ещё больше.")
