...
def попытка(i, номер, сид, густой=None):
    if густой is None:
        густой = номер in ГУСТЫЕ
    d = песочница(i)
    синхр(d)
    папка = os.path.join(БАЗА, 'try%d-%d' % (i, сид))
    shutil.rmtree(папка, ignore_errors=True)
    try:
        # Имя набора передаём явно: попытки собираются во временные папки, и
        # без этого в журнал владельцем уходило имя вида try0-43032.
        r = php('engine/generator-v5.php', '--выход=' + папка, '--сид=%d' % сид,
                '--данные=' + d, '--тихо', '--имя=nabor-%d' % номер,
                '--манера=' + ('густая' if густой else 'обычная'))
...
# Потолок попыток на набор. Густая манера при среднем 3.00 промаха даёт
# чистый набор примерно раз в сотню попыток, и хвост у такого распределения
# длинный: набор 701 не сошёлся за 1160 попыток (пять с половиной часов),
# тогда как 698 взялся за 28, а 703 за 104 на более истощённом журнале. Без
# потолка партия молча стоит часами; с потолком драйвер меняет манеру на
# обычную и идёт дальше, о чём пишет в журнал.
ПОТОЛОК_ПОПЫТОК = int(os.environ.get('V5_ПОТОЛОК', '400'))

def набор(номер, сид0, журнал, густой=None):
    попыток = 0
    сид = сид0
    if густой is None:
        густой = номер in ГУСТЫЕ
    while попыток < ПОТОЛОК_ПОПЫТОК:
        задания = [(i, номер, сид + i, густой) for i in range(РАБОТНИКОВ)]
        ...
        сид += РАБОТНИКОВ
    if густой:
        журнал.write('набор %d: густая манера не сошлась за %d попыток, беру обычную\n'
                     % (номер, попыток))
        журнал.flush()
        с, п = набор(номер, сид, журнал, густой=False)
        return с, попыток + п
