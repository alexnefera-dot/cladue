...
final class Stylistics
{
    ...
    public static function of(TextMetrics $tm, SeoMetrics $seo, string $brand, array $brandNames): array
    {
        ...
        // Формы на «-ьте» мера не видела вовсе: «проверьте», «сверьте»,
        // «поставьте», «отметьте» — то есть самые частые в этом жанре.
        // Возвратные «дождитесь» и «убедитесь» тоже проходили мимо.
        $imper = preg_match_all('/\b[\p{L}]+(?:йте|ите|ьте)(?:сь)?\b/u', $low);
