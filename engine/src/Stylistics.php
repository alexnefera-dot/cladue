...
{
    /** Слова на «-йте/-ите/-ьте», которые императивами не являются. */
    private const NE_IMPERATIV = ['сайте', 'интернете', 'комитете', 'пакете',
        'кабинете', 'билете', 'буфете', 'сюжете', 'бюджете', 'этикете',
        'хотите', 'предмете', 'декрете', 'портрете', 'секрете', 'браслете',
        'планете', 'жилете', 'манжете', 'пикете', 'лете', 'свете', 'цвете'];
    ...
    public static function of(TextMetrics $tm, SeoMetrics $seo, string $brand, array $brandNames): array
    {
        ...
        $secondP = preg_match_all('/\b(вы|вас|вам|вами|ваш|ваша|ваши|ваших|вашего)\b/u', $low);
        // Императив: «-йте/-ите/-ьте». «-ьте» тут не роскошь — «проверьте»
        // самая частая вежливая форма жанра (189 вхождений в корпусе), и без
        // неё счётчик не видел ни её, ни «будьте», ни «не забудьте».
        // «Сайте», «интернете», «пакете» под правило подходят по форме, но
        // императивами не являются: 373 вхождения одного «сайте» — почти
        // десятая часть всего счёта, и их приходится вычитать списком.
        $imper = preg_match_all('/\b[\p{L}]+(?:йте|ите|ьте)\b/u', $low, $impM);
        $imper -= count(array_filter($impM[0],
            static fn(string $w): bool => in_array($w, self::NE_IMPERATIV, true)));
