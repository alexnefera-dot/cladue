...
final class Planner
{
    ...
    public function plan(string $type, array $brand, ?StyleProfile $style = null, ?array $donor = null): array
    {
        ...
        // Корпоративное «мы» — самостоятельная цель, а не производная от первого
        // лица. Первый прогон v3: донор хочет 6, вышло 23; другой хочет 5, вышло
        ...
        // Абзацы и их длина. Поле paragraphs жило в профиле с корпуса v1 и целью
        // никогда не было: замер связки дал 448 абзацев по 36 слов у референса
        // против 792 по 27 у генерации. Данных при этом столько же — они просто
    ...
    private function buildLinks(string $type, Rng $rng, string $brandRu, string $brandEn, ?array $donor = null): array
    {
        ...
        if ($this->pages !== []) {
            $paths = [];
            // Переменная цикла НЕ должна называться $cfg: она затирала конфиг
            // перелинковки, и связка выходила с нулём ссылок при цели 118.
            foreach ($this->pages as $pt => $pcfg) { $paths[$pt] = $pcfg['path'] ?? ('/' . $pt); }
        }
