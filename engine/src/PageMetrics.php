...
final class PageMetrics
{
    ...
    /**
     * Те же формулы словами — для брифа на правку. Писателю приходило
     * «anchors 1→5»: имя поля из карточки, по которому нельзя понять ни что
     * это, ни что дописать. Список держится рядом с регулярками, чтобы они не
     * разъехались.
     */
    public const ANCHORS_TXT = [
        'официальный сайт', 'игровые автоматы', 'бонусы и промокоды', 'можно ли',
        'личный кабинет', 'зеркало сайта', 'вывод средств', 'ответы на', 'вход в',
    ];
    ...
    public static function measure(Analyzer $a, string $type, string $raw, array $brand = ['ru' => '', 'en' => '']): array
    {
        ...
        // Глагольная часть — второе лицо единственного числа: «смотришь»,
        // «поставишь». Слово «лишь» подходит под тот же хвост и не является
        // обращением: без исключения оно давало ty 3 при цели 0 на странице,
        // где на «ты» не переходили ни разу. Список исключений короткий —
        // других частотных не-глаголов на «-ешь/-ишь» в русском нет.
        $tyIsklyuch = '(?:лишь|вишь)';
        $ty = preg_match_all('~\b(ты|тебе|тебя|тобой|твой|твоя|твоё|твои|твоего|твоей|твоих|твоим)\b'
            . '|\b(?!' . $tyIsklyuch . '\b)\w+(?:ешь|ишь)\b~ui', $fullText);
