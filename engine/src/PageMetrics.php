...
final class PageMetrics
{
    ...
    public const FIELDS = [
        ...
        'ty' => ['«ты»', 0],
        'vy_share' => ['доля «вы» среди обращений, %', 1],
        'address_total' => ['обращений к читателю', 0],
    ...
    public static function measure(Analyzer $a, string $type, string $raw, array $brand = ['ru' => '', 'en' => '']): array
    {
        ...
        // Регистр обращения долей, а не счётом. Счётчики «вы» и «ты» устроены
        // по-разному: «ты» считает и местоимения, и глагольные формы на
        // «-ешь/-ишь», «вы» — только местоимения, и сравнивать их между собой
        // нельзя. Здесь оба берутся местоимениями, поэтому перевод «ты» → «вы»
        // переносит единицу из одной чаши в другую и суммы не меняет: доля —
        // единственная величина этой пары, которую можно и померить, и выставить.
        $мест = fn(string $re): int => preg_match_all($re, $fullText);
        $тыМест = $мест('~\b(ты|тебе|тебя|тобой|тобою|твой|твоя|твоё|твое|твои|твоего'
            . '|твоей|твоему|твоим|твоими|твоих|твою|твоём|твоем)\b~ui');
        $выМест = $мест('~\b(вы|вас|вам|вами|ваш|ваша|ваше|ваши|вашего|вашей|вашему'
            . '|вашим|вашими|ваших|вашу|вашем)\b~ui');
        $обращений = $тыМест + $выМест;
        ...
        return [
            ...
            'anchors' => $anchors,
            'ty' => $ty,
            'vy_share' => $обращений >= 4 ? round($выМест / $обращений * 100, 1) : 0.0,
            'address_total' => $обращений,
