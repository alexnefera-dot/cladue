...
/**
 * Поля с односторонним полом: их держит не коридор, а нижняя граница.
 *
 * Приёмка, задание и построчный замер читают `пол_рынка` уже давно, а записать
 * его было некому: ключа не было ни в одном профиле. Раздел «полы по рынку»
 * всё это время печатал «26/0» и проходил всегда — шесть проверок комплекта
 * работали, седьмая была декорацией. Нашлось чтением готовой страницы вслух:
 * повелительных на ней оказалось впятеро меньше, чем требует задание, и никто
 * не возразил.
 *
 * Пол — ПЕРВЫЙ ДЕЦИЛЬ корпуса: ниже, чем у девяти доноров из десяти, это уже
 * не разброс жанра. Медиану в пол ставить нельзя — она завернёт половину
 * самого корпуса.
 */
const POLY_POLA = ['terms_total', 'first_person', 'imperatives', 'vy', 'honest', 'para_one_sent_pct'];
...
foreach (TIPY as $tip) {
    ...
    foreach (PageMetrics::FIELDS as $k => [$label, $isRate]) {
        ...
        $polya[$k] = [
            ...
            'пол_рынка' => in_array($k, POLY_POLA, true) ? round($v[(int) ($n * 0.1)], 2) : 0,
