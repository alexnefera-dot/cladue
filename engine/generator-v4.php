...
if ($krc === 0 && !$SUHOY) {
    ...
    foreach ($karta['срезы'] as $p2 => $srez) {
        // Реестр хранит пары «срез — комплект»: по ним задание узнаёт СВОЙ
        // резерв при перевыпуске. Сверять надо по срезу внутри пары, иначе
        // строка ляжет вторым, уже безымянным экземпляром, и перевыпуск
        // комплекта уйдёт занимать следующий свободный срез вместо своего.
        $est = false;
        foreach ($mk['занято']['срезы'][$p2] ?? [] as $z) {
            if ((is_array($z) ? ($z['срез'] ?? '') : $z) === $srez) { $est = true; break; }
        }
        if (!$est) { $mk['занято']['срезы'][$p2][] = ['срез' => $srez, 'комплект' => $IMYA]; }
