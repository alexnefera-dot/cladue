...
// Эхо тела в ответах FAQ. Нашлось сплошным чтением: короткие абзацы в одно
// предложение, поставленные ради пола «абзац-фраза», были собраны из ответов
// FAQ той же страницы — сначала выжимка, следом полный ответ теми же словами.
//
// Порог взят из рынка и он жёсткий. У 50 доноров таких ответов ноль на 1992,
// то есть повтор здесь не редкость, а прямой запрет жанра: чужой FAQ отвечает
// на то, чего в теле нет вовсе. Считаем по общим словам от пяти букв, долю
// берём от меньшего множества — так короткая выжимка из длинного абзаца видна
// так же хорошо, как две одинаковые фразы.
$ekho = Ekho::proverit($html);
printf("\n── эхо тела в ответах ──\n");
echo '  ' . ($ekho ? '✗' : '·') . ' ' . $pad('ответов-пересказов', 30, true)
    . $pad((string) count($ekho), 12) . "   нужно 0 (у доноров 0 из 1992)\n";
foreach (array_slice($ekho, 0, 5) as $e) {
    printf("      · ответ «%s…»\n", mb_substr($e['ответ'], 0, 64));
