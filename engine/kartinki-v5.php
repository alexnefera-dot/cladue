...
'main' => [212, ['барабаны','лента'], [
    ...
    ['каталог игр: слоты, столы и live', 'лента турниров и акций на главной странице'],
    ['навигация по разделам', 'обзор стартового экрана площадки']]],
...
'obzor' => [268, ['диаграмма','щит'], [
    ...
    ['обзор казино: на что смотреть при выборе', 'сравнение скорости выплат и условий отыгрыша'],
    ['критерии оценки площадки', 'показатели надёжности и работы поддержки'],
    ['проверка условий вывода средств в', 'разбор лицензии и правил площадки']]],
...
'promo' => [340, ['купон','монеты'], [
    ...
    ['промокоды и акции казино', 'как проверить срок действия промокода'],
    ['актуальные акции недели в', 'где публикуют свежие промокоды'],
...
'news' => [196, ['лента','график'], [
    ...
    ['свежие обновления и события', 'график публикации новостей площадки'],
...
'info' => [186, ['щит','форма'], [
    ...
    ['лицензия и правила площадки', 'этапы верификации аккаунта перед выводом'],
    ['защита данных игроков в', 'какие документы просят при проверке'],
    ['порядок работы площадки', 'правила, верификация и обращение в поддержку']]],
...
'partnery' => [154, ['график','диаграмма'], [
    ...
    ['партнёрская программа: схема отчислений', 'статистика по кликам, депозитам и доходу'],
    ['как формируется доход партнёра', 'зависимость процента от качества трафика'],
    ['кабинет партнёра со статистикой', 'отчёт по кликам, регистрациям и выплатам']]],
...
'app' => [228, ['телефон','форма'], [
    ...
    ['установка приложения на телефон', 'вход в аккаунт через мобильное приложение'],
    ['интерфейс приложения', 'разделы игр и касса в мобильной версии']]],

'bonus' => [42, ['монеты','купон'], [
    ['Бонусы', 'Вейджер и отыгрыш'],
    ['Подарок к депозиту', 'Процент и условия'],
    ['Бонусный баланс', 'Путь до вывода']], [
    ['бонусы и вейджер в казино', 'как считается отыгрыш бонусного баланса'],
    ['подарок к первому депозиту в', 'процент бонуса и требования по отыгрышу'],
    ['бонусный баланс', 'путь от начисления бонуса до выплаты']]],
...
'registracia' => [174, ['форма','ключ'], [
    ...
    ['форма регистрации аккаунта в', 'шаги создания аккаунта и подтверждения почты'],
    ['создание аккаунта в за минуту', 'подтверждение телефона при регистрации'],
    ['поля анкеты при регистрации в', 'что заполнять перед первым депозитом']]],
...
'slots' => [284, ['барабаны','диаграмма'], [
    ...
    ['характеристики слотов в', 'волатильность и частота выплат автомата'],
    ['каталог автоматов', 'показатель отдачи на длинной дистанции']]],
...
'vhod' => [206, ['ключ','телефон'], [
    ...
    ['доступ к аккаунту', 'подтверждение входа и смена пароля'],
    ['личный кабинет игрока', 'баланс, история ставок и раздел кассы']]],
...
'zerkalo' => [16, ['зеркало','щит'], [
    ...
    ['официальное зеркало', 'как отличить настоящий адрес от подделки']]],
];
