...
/**
 * ── правка 0: заголовок с нулём вместо имени ─────────────────────────
 *
 * В корпусе остались плейсхолдеры чужой шаблонизации — `{NAME}`, `{AMOUNT}`.
 * Подстановка их не знала и писала ноль: «Джекпот слоты — 0 сорвал 0 здесь».
 * Само заполнение уже исправлено, но в собранных наборах заголовки такими и
 * остались, а реалайзер H2 не трогает — чинить их некому, кроме доводчика.
 */
function v5ПравкаЗаголовков(string $html, array $паспорт, array $банки): array
{
    $сделано = 0;
    $html = preg_replace_callback('~(<h2>\s*)(.*?)(\s*</h2>)~s',
        function ($m) use (&$сделано, $паспорт, $банки) {
            if (!preg_match('~(^|\s)0(\s|$)~u', $m[2])) { return $m[0]; }
            // Ноль после «сорвал», «заработал», «выиграл» — это сумма, любой
            // другой — имя. Сумма ищется первой, иначе имя забирает оба.
            $t = preg_replace('~(?<=сорвал |заработал |выиграл )0\b~ui', '{СУММА}', $m[2]);
            $t = preg_replace('~(^|\s)0(?=\s|$)~u', '$1{ИМЯ}', $t);
            if ($t === $m[2]) { return $m[0]; }
            $сделано++;
            return $m[1] . v5Заполнить($t, $паспорт, $банки) . $m[3];
        }, $html);
    return [$html, $сделано];
}
...
foreach ($файлы as $тип => $f) {
    ...
    // 0. заголовки с нулём вместо имени
    [$новый, $n] = v5ПравкаЗаголовков($html, $паспортНабора, $пулы['общие']['банки']);
    if ($n) {
        $html = $новый;
        printf("%-13s %-16s %8s %8s %8s  %s\n", $тип, 'h2', '0', '—', 'имя', "заголовков: $n");
    }
