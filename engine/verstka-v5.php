...
$ПРАВИЛА = [
    // ── шапка страницы
    'hero-value'        => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:16px;background:' . ПОДЛОЖКА . ';padding:20px;margin:0 0 22px',
    ...
    'hero-headline'     => 'font-weight:700;margin:12px 0 6px;font-size:26px;line-height:1.25',
    ...
    'hero-quicklinks'   => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;margin:14px 0 0;padding:0;list-style:none',
    'quicklinks-title'  => 'font-weight:700;margin:22px 0 12px;font-size:18px',
    ...
    'quicklink-item'    => 'display:grid;grid-template-columns:26px minmax(0,1fr);column-gap:10px;align-items:center;min-width:0;padding:12px 14px;border:' . КАНТ . ';border-radius:12px;background:' . ПОДЛОЖКА . ';text-decoration:none;color:inherit',
    'ql-icon'           => 'grid-column:1;grid-row:1/span 2;font-size:20px;line-height:1;white-space:nowrap',
    'ql-text'           => 'grid-column:2;min-width:0;font-weight:600;line-height:1.3',
    'ql-meta'           => 'grid-column:2;font-size:12px;opacity:.84;margin-top:2px',

    // ── последние выплаты: вид таблицы без тега table
    'recent-payouts-block' => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:14px;overflow:hidden;margin:0 0 22px',
    ...
    'jackpot-strip-icon' => 'font-size:18px;line-height:1;flex:0 0 auto',
    ...
    // ── преимущества
    'value-pillars'      => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;margin:0 0 22px',
    'value-pillars-heading' => 'font-weight:700;margin:0 0 12px;font-size:19px',
    'value-pillars-grid' => 'display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px',
    'value-pillar'       => 'display:block;min-width:0;padding:14px;border:' . КАНТ . ';border-radius:12px;background:' . ПОДЛОЖКА . '',
    'value-pillar-icon'  => 'display:block;font-size:24px;line-height:1;margin-bottom:8px;white-space:nowrap',
    ...
    // ── отзывы
    'review-quotes-block' => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;margin:0 0 22px',
    'review-quotes-heading' => 'font-weight:700;margin:0 0 6px;font-size:19px',
    ...
    'review-quote-text'  => 'display:block;min-width:0;padding:14px 14px 12px;border:' . КАНТ . ';border-left:3px solid ' . ЗОЛОТО . ';border-radius:0 12px 12px 0;background:' . ПОДЛОЖКА . ';font-style:italic;line-height:1.55',
    'review-quote-summary' => 'display:block;margin-top:8px;font-size:12px;font-style:normal;opacity:.84',

    // ── вопросы и ответы
    'faq-section'   => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:14px;background:' . ПОДЛОЖКА . ';padding:18px;margin:22px 0',
    'faq-list'      => 'display:grid;gap:10px;margin:12px 0 0;padding:0;list-style:none',
    'faq-item'      => 'display:block;min-width:0;padding:12px 14px;border:' . КАНТ . ';border-radius:12px;background:' . ПОДЛОЖКА2 . '',
    ...
    'layout-block'        => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;margin:0 0 22px',
    'page-thematic-block' => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:14px;background:' . ПОДЛОЖКА . ';padding:18px',
    'info-block'          => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:14px;background:' . ПОДЛОЖКА . ';padding:18px;margin:0 0 22px',
    'gradient-text'       => 'font-weight:700;display:block;margin:0 0 10px;font-size:20px',
    'text-glow-animation' => 'font-weight:700;display:block;margin:0 0 10px;font-size:20px',

    // ── дашборд слотов
    'slots-dashboard'           => '' . ШРИФТ . ';line-height:1.5;text-align:left;display:block;border:' . КАНТ . ';border-radius:16px;background:' . ПОДЛОЖКА . ';padding:18px;margin:0 0 22px',
    ...
    'slots-dashboard-icon'      => 'font-size:22px;line-height:1;flex:0 0 auto',
    'slots-dashboard-title-wrap' => 'display:block;min-width:0',
    'slots-dashboard-title'     => 'font-weight:700;margin:0;font-size:19px',
...
// Ничего от шаблона сайта: у каждого правила есть свой цвет, шрифт и выключка
// (inherit тянет из нашего же контейнера, а инлайн бьёт селекторы хоста —
// `span{color}`, `p{text-align:justify}`, `h3{text-transform}`).
foreach ($ПРАВИЛА as $к => $v) {
    if (strpos($v, 'color:') === false) { $v .= ';color:inherit'; }
    if (strpos($v, 'font-family:') === false) { $v .= ';font-family:inherit'; }
    if (strpos($v, 'text-align:') === false) { $v .= ';text-align:inherit'; }
    if (strpos($v, 'text-transform:') === false && in_array($к, ['hero-headline', 'quicklinks-title', 'value-pillars-heading', 'review-quotes-heading',
        'gradient-text', 'text-glow-animation', 'slots-dashboard-title', 'slot-name', 'jackpot-strip-title', 'payout-feed-title', 'value-pillar-title', 'faq-question'], true)) {
...
if ($безКартинок) {
    ...
    $ПРАВИЛА['slot-fallback-icon'] = 'font-size:52px;line-height:1;white-space:nowrap';
...
$КОЛОНКИ = 'display:grid;grid-template-columns:34px minmax(90px,1fr) minmax(96px,auto)'
         ...
         . ';letter-spacing:.04em;text-transform:uppercase;opacity:.9;font-weight:600;color:inherit;font-family:inherit';
...
foreach ($файлы as $файл) {
    ...
    foreach ($строки as $i => $л) {
        ...
        if (preg_match('~^\s*<p[\s>]~', $л) === 1 && isset($строки[$i + 1])) {
            ...
            if (strpos($тело, '<strong>') === 0 && mb_strlen($низ, 'UTF-8') > 40 && strpos($л, 'style=') === false) {
                ...
                if ($фон !== '') {
                    $стиль = ШРИФТ . ';font-weight:400;margin:16px 0;padding:12px 16px;border-left:4px solid ' . $цвет
                           . ';border-radius:0 10px 10px 0;background:' . $фон . ';line-height:1.6;text-align:left';
        ...
        // заголовок может нести id (оглавление): атрибуты сохраняем
        if (preg_match('~^\s*<(h2|h3)(\s(?![^>]*class=)[^>]*)?>\s*$~', $л, $mz) === 1) {
            $итог[] = preg_replace('~<' . $mz[1] . '(\s[^>]*)?>~', '<' . $mz[1] . '$1 style="' . v5TemaZagolovok($Т, $mz[1]) . '">', $л, 1);
            $правил++;
            continue;
        }
        if (preg_match('~^\s*<p>~', $л) === 1) {   // абзац может начинаться текстом в той же строке
            $итог[] = preg_replace('~<p>~', '<p style="' . ШРИФТ . ';font-weight:400;text-align:left;line-height:' . $Т['строка'] . ';margin:0 0 14px">', $л, 1);
            $правил++;
            continue;
        }
        // ── содержательные списки: воздух и цветные маркеры
        if (preg_match('~^\\s*<ul>\\s*$~', $л) === 1) {
            $итог[] = preg_replace('~<ul>~', '<ul style="' . ШРИФТ . ';font-weight:400;text-align:left;margin:14px 0;padding-left:22px;line-height:' . $Т['строка'] . ';list-style-type:' . $Т['маркер'] . '">', $л, 1);
            $правил++;
            continue;
        }
        if (preg_match('~^\\s*<li>~', $л) === 1) {
            $итог[] = preg_replace('~<li>~', '<li style="margin:0 0 8px;padding:0;color:inherit;font:inherit;text-align:inherit">', $л, 1);
            $правил++;
            continue;
        }
        ...
        elseif ($вЧипах && preg_match('~^(\s*)<li(\s|>)~', $л, $m2) === 1 && strpos($л, 'style=') === false) {
            $итог[] = preg_replace('~^(\s*<li)(\s|>)~',
                ...
                . КАНТ . ';border-radius:999px;background:' . ПОДЛОЖКА . ';font-size:13px;color:inherit;font-family:inherit;text-align:left"$2', $л, 1);
            $правил++;
            continue;
        }
    ...
    // последний проход: всё, что осталось без style=, получает жёсткий сброс
    foreach ($итог as $k => $л) { $итог[$k] = v5ZhyostkoStroka($л, $Т, $правил); }
