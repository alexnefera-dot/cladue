...
foreach ($chunks as $bi => $chunk) {
    ...
    // Компенсация нужна только там, где плотность низкая: блок не видит чисел
    // соседей и тратит свой бюджет целиком, отсюда перебор на склейке. На
    // плотных донорах (замер: цель 15.4, вышло 6) она наоборот углубляет
    // недобор, поэтому применяется только ниже порога.
    if (isset($sub['targets']['numbers_per100']) && (float) $sub['targets']['numbers_per100'] < 6.0) {
