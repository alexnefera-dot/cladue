        ...
        foreach ($р['фразы'] as $ф) {
            foreach (preg_split('~(?<=[.!?…])\s+~u', $ф) as $пр) {
                ...
                if (preg_match('~\d[\d\s]*\s*(?:₽|руб|к\b|тыс|млн)|\b(?:он|она|его|её|ему|ей)\b|\b(?:минут|часа|часов|секунд)\b~u', $пр)
                    && preg_match('~\b(?:вывел|вывела|забрал|снял|поднял|принёс|принесла|потратил|ушло|висело|лежало|светил[а-яё]*|показывал|плюс|минус|проиграл|слил|через|было|стало|оказалось|капнуло|пришло|упало|перерыв[а-яё]*|ждал|ждала)\b~ui', $пр)
