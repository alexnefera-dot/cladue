...
test('демо-сид: сегодня, трекинг, прогнозы и имущество живые', () => {
  ...
  assert.equal(c('SELECT count(*) AS c FROM metrics'), 11, '3 числовых примера + 8 колонок дневника');
  assert.equal(c(`SELECT count(*) AS c FROM metrics WHERE type = 'bool'`), 8, 'колонки из гугл-таблицы');
  assert.equal(c(`SELECT count(*) AS c FROM metrics WHERE name = 'Подъем не в 10'`), 0, 'исключённая колонка вычищена');
