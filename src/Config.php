...
final class Config
{
    ...
    public static function defaults(): array
    {
        return [
            ...
            'search' => [
                // Сколько запросов выдачи тянуть ПАРАЛЛЕЛЬНО (1 — по одному, как раньше). Список из тысяч
                // ключей по одному запросу за раз собирается часами; источник (API/XMLStock) допускает
                // несколько потоков. На живую выдачу через прокси не влияет — там строгий порядок.
                'concurrency' => 5,
                'region' => 213,
                'search_type' => 'ru',
                'l10n' => 'ru',
            ...
            'visit' => [
                ...
                'similarity' => 0.9,
                // Пауза после загрузки страницы перед снимком: сетевую активность уже дождались
                // (networkidle), поэтому долго ждать незачем — это время умножается на число страниц.
                'wait_ms' => 1200,
                'timeout' => 30,
                // Сколько раз повторить загрузку страницы при таймауте/сетевой ошибке — каждый раз
                // через другой прокси и с увеличенным таймаутом (0 — без повторов)
                'retries' => 2,
                // Менять ли User-Agent на повторе: часть сайтов закрыта именно от робота поисковика
                // (403/антибот-заглушка), и та же страница спокойно открывается под браузером
                'retry_user_agents' => true,
                // Сколько ЕЩЁ раз пробовать открыть главную у сайтов, где превью так и не получилось
                // («без превью» в таблице). Каждая попытка — другой прокси И другой браузерный агент,
                // таймаут растёт; 0 — не пробовать
                'preview_retries' => 2,
                // Распознавать «подборку офферов» — витрину чужих бонусов вместо настоящего сайта:
                // такая страница не сохраняется как контент, а перезапрашивается с другого IP и агента
                'detect_offer_walls' => true,
                // Добирать ключевые страницы (регистрация, вход, зеркало, бонусы, приложение, слоты) по
                // стандартным адресам, если ссылки на них не нашлось в меню. Включает кнопка «Добрать
                // ключевые» в панели — обычная докачка лишних запросов не делает.
                'retry_key_pages' => false,
                // Сколько страниц открывать одновременно в одном браузере и сколько браузеров (по одному
                // на прокси) запускать параллельно: с десятком прокси выгрузка идёт в browsers×concurrency потоков.
                'concurrency' => 4,
                'browsers' => 2,
                // Пауза между заходами считается ОТДЕЛЬНО для каждого прокси и каждого сайта: разные сайты
                // открываются параллельно, а один сайт и один прокси не заваливаются запросами.
                'delay_ms' => 400,
