...
final class SiteLinks
{
    ...
    public static function canonical(string $url): string
    {
        ...
        // Путь целиком из языкового кода — это та же главная, сворачиваем к корню (иначе получаем
        // дубль main). Форму xx-xx (/ru-ru, /en-us) считаем языком всегда; одиночный код — только из
        // известного списка, чтобы не принять реальную короткую страницу за язык.
        if (preg_match('~^/[a-z]{2}-[a-z]{2}$~i', $path)
            || preg_match('~^/(?:ru|en|ua|uk|be|by|kz|uz|az|kg|tj|tm|am|ge|md|ro|pl|de|fr|es|pt|tr)$~i', $path)) {
            $path = '';
        }
        if ($path === '') {
            $path = '/';
