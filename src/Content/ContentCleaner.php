...
final class ContentCleaner
{
    ...
    /** Имена для чужого незаполненного плейсхолдера {NAME} («Где {NAME} выигрывает чаще»): в каждом месте — случайное. */
    public const RANDOM_NAMES = [
        'Алексей', 'Дмитрий', 'Сергей', 'Иван', 'Максим', 'Андрей', 'Николай', 'Егор', 'Артём', 'Павел',
        'Михаил', 'Владимир', 'Роман', 'Кирилл', 'Илья', 'Олег', 'Антон', 'Денис', 'Виктор', 'Георгий',
        'Игорь', 'Евгений', 'Константин', 'Станислав', 'Тимур', 'Руслан', 'Ярослав', 'Даниил', 'Глеб', 'Никита',
    ];
    ...
    public function applyReplacements(string $html, array $opt): string
    {
        ...
        // Чужой плейсхолдер {NAME} (незаполненный шаблон донора) — случайное имя, в каждом месте своё;
        // это не наша переменная бренда, поэтому подставляем готовое имя, а не %…%.
        $html = preg_replace_callback('~\{NAME\}~i', static fn (): string => self::RANDOM_NAMES[random_int(0, count(self::RANDOM_NAMES) - 1)], $html) ?? $html;
        return $html;
    }
