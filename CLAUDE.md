...
- DOMAINS HOLDING MANY BRANDS — «домены, которые встречаются более чем в 5 разных брендов (это именно отобранные после
  ...
  after the FILLER words — at most two words of ≥ 3 chars, so «пластиковые окна москва цена» is not a
  brand. Spellings fold in two steps: `PAIRS` (166 EN/RU pairs the user supplied, `const` in the class,
  extendable through the gitignored `brand-pairs.txt` — one spelling per line, latin then cyrillic) cover
