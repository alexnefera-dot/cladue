...
final class PipboySchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    private func database() throws -> Database {
        ...
        // Служебные старт-операции (миграции, дедуп, unapplyTargetMoves, дефолты) не должны омолаживать updated_at:
...
enum Api {
    ...
    static func write(method: String, path: String, body: [String: Any], db: Database) throws -> (Data, Int) {
        ...
        if method == "POST", path == "/api/rest/seed" {
            ...
            let pack: [(String, String, String, Int)] = [
                ...
                ("Кофеин до полудня", "restore", "weekday", 0),
                ("Термалка — их рядом полно и это дёшево", "restore", "weekend", 180),
                ("Сауна", "restore", "weekend", 120),
                ("Массаж", "restore", "weekend", 90),
                ("Выспаться без будильника — один раз, а не два", "restore", "weekend", 0),
                ("День без auto.ria и без подсчёта портфеля", "restore", "weekend", 0),
                ("Анализы: ферритин, железо, D, B12, ТТГ, Т4", "restore", "global", 0),
                ("Найти терапевта под ГТР", "restore", "global", 0),
    ...
    // Разово откатывает переносы, ранее вписанные в value (обратный знак к прежнему поведению).
    static func unapplyTargetMoves(_ db: Database) {
