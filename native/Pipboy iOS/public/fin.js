...
function portRows(it, depth, ctx) {
  ...
  let cells;
  {
    ...
    // «Станет» заполнено всегда: без переносов равно «Сейчас». Так колонку можно читать
    // сверху вниз и сверять с «Целью», а перенос выделяется стрелкой и цветом.
    const becameShown = editable ? fmt((it.value || 0) + netCur) : fmtE((it.eur || 0) + net);
    const becameCell = `<td class="r num became">${net === 0 ? `<span class="quiet">${becameShown}</span>`
      : `<span class="${net > 0 ? 'up' : 'down'}" title="сейчас ${editable ? fmt(it.value || 0) : fmtE(it.eur)} ${net > 0 ? '+' : '−'} ${fmt(Math.abs(editable ? netCur : net))} переносами">→ ${becameShown}</span>`}</td>`;
  ...
  <div class="card">
    ${finIsMobile()
      ...
      <tr><th>Название</th>
        ...
        <th class="r" style="width:104px" title="сейчас плюс запланированные переносы; без них равно «Сейчас»">Станет</th>
  ...
  document.querySelectorAll('[data-fe]').forEach(el =>
    el.addEventListener('click', () => {
      ...
      inlineVal(el, type, async v => {
        ...
        // Баланс счёта — единственное место, где расхождение с итогом сверху было бы незаметно:
        // проверяем, что значение реально легло в базу, а не отброшено по дороге.
        if (ent === 'accounts' && field === 'balance') {
          const acc = ((await finApi.list()).accounts || []).find(x => x.id === +id);
          if (acc && Math.abs((acc.balance ?? 0) - v) > 0.005)
            alert(`Баланс не сохранился: в базе ${fmt(acc.balance)}, вписано ${fmt(v)}.`
              + ' Похоже, приложение собрано до этой правки — пересобери в Xcode (⌘R).');
        }
